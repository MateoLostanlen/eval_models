...
def run_engine_predictions(image_folder, outpath, dConfig, save_pred=True, resume=True):
    """
    ...
    for configId, config in dConfig.items():
        ...
        for label in ["fp", "fire"]:
            for session in tqdm(os.listdir(os.path.join(image_folder, label))):
                ...
                # Nex instance of pyro engine for each image sequence
                pyroEngine = Engine(
                ...
                for file in glob.glob(f"{os.path.join(image_folder, label, session)}/*"):
                    im = Image.open(file)
